...
# ===================================================================================
# ==     精简版 XFCE 桌面（用于终端访问）一键式无密码启动脚本 v1.0         ==
# ===================================================================================
# == 作者: Kilo Code (经 Gemini 精简与优化)                                        ==
# == 功能: 在 Debian/Ubuntu 系统上，一键安装并以无密码模式启动一个最小化的桌面环境。 ==
# ==       1. 安装 XFCE 核心组件与终端。                                       ==
# ==       2. 安装 TigerVNC (VNC 服务端) 和 noVNC (Web 客户端)。               ==
# ==       3. 脚本可安全重复运行，自动清理旧服务，确保稳定启动。                 ==
# ===================================================================================
#
# == ⚠️ 安全警告: 此脚本配置的VNC服务没有密码，请仅在绝对受信任的网络环境中使用！ ==
#
# == 使用方法:
# ==   1. 将此完整代码保存为 minimal_desktop.sh
# ==   2. 赋予执行权限: chmod +x minimal_desktop.sh
# ==   3. 使用 root 权限运行: sudo ./minimal_desktop.sh
#
# ===================================================================================
...
VNC_GEOMETRY="1280x800"           # 桌面分辨率 (可按需调整)
VNC_DEPTH="24"                    # 颜色深度
...
# --- 安装完成标记文件 ---
INSTALL_FLAG_FILE="/etc/minimal_desktop.installed"
...
install_environment() {
    log_info "====================================================="
    log_info "==     首次运行，开始执行最小化环境安装流程...       =="
    log_info "====================================================="
    export DEBIAN_FRONTEND=noninteractive
    apt-get update -y
    
    # 只安装核心组件：sudo, curl(用于获取IP), dbus-x11(改善桌面稳定性)
    # xfce4-terminal(终端), xfce4(核心桌面)
    # tigervnc, novnc, websockify (VNC 核心服务)
    ...
    if ! id "$VNC_USER" &>/dev/null; then
        ...
        adduser "$VNC_USER" sudo # 将用户添加到 sudo 组，方便管理
    fi
