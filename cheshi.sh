    ...
    if [[ -n "$IPv6_ADDR" ]]; then
        ...
        get_google_ipv6() {
            ...
            get_ipv6_with_nslookup() {
                nslookup -type=AAAA google.com 2>/dev/null | grep -E "$ipv6_regex" | head -n 1 | awk '{print $NF}'
            }

            get_ipv6_with_host() {
                host -t AAAA google.com 2>/dev/null | grep -E "$ipv6_regex" | head -n 1 | awk '{print $NF}'
            }
